...
**Project skill** (`.claude/skills/agent-creation-orchestration/`) — loads automatically for any Claude Code session working in this repository.
Corpus: bundled `INDEX.md` + `digests/`. Prefer digests over re-scraping Medium.

> To make this skill available in **every** repository on a machine (true global scope), copy this folder to `~/.claude/skills/agent-creation-orchestration/` on that machine — user-level skills there apply across all projects. Skills committed under a repo's `.claude/skills/` apply to that repo for anyone who checks it out.
